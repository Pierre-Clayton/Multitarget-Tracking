...
def initialize_target_state():
    position = cp.array([
        cp.random.uniform(0, L_x),  # x position
        cp.random.uniform(0, L_y)   # y position
    ])
    velocity = cp.array([
        cp.random.uniform(-V_max, V_max),  # x velocity
        cp.random.uniform(-V_max, V_max)   # y velocity
    ])
    return cp.concatenate((position, velocity))
...
R = sigma_measurement ** 2 * cp.eye(2)  # Measurement noise covariance matrix
...
target_states = [cp.zeros(4) for _ in range(N_max)]  # State: [x, y, vx, vy]
...
for _ in range(N_p):
    x_k_init = cp.array([initialize_target_state() if cp.random.rand() < P_B else x_death for _ in range(N_max)])
    particle = {
        #'x_k': cp.array([initialize_target_state() if cp.random.rand() < P_B else x_death for _ in range(N_max)]),  # State of all targets

        'x_k': x_k_init,  # State of all targets

        #'e_k': cp.array([1 if cp.random.rand() < P_B else 0 for _ in range(N_max)], dtype=int),  # Existence flags of all targets

        'e_k': cp.array([[1] if x_k_init[i][0] != 0 and x_k_init[i][1] != 0 else [0] for i in range(N_max)], dtype=int), # Existence flags of all targets

        #'x_k_prev': cp.zeros((N_max, 4)),  # Previous state
        'x_k_prev': x_k_init,  # define initial previous state as the initial state

        #'e_k_prev': cp.zeros(N_max, dtype=int),  # Previous existence flags
        'e_k_prev': cp.array([[1] if x_k_init[i][0] != 0 and x_k_init[i][1] != 0 else [0] for i in range(N_max)], dtype=int), # initialize e_k_prev to be equal to e_k
...
def update_true_targets(k, target_states, existence_flags, target_schedule):  #this functions returns nothing, could be problematic
    for n in range(N_max):
        ...
        if birth_time <= k < death_time:
            ...
            process_noise = cp.random.multivariate_normal(mean=cp.zeros(4), cov=Q)
            target_states[n] = A @ target_states[n] + process_noise
            # Ensure target stays within surveillance area
            target_states[n][0] = cp.clip(target_states[n][0], 0, L_x)
            target_states[n][1] = cp.clip(target_states[n][1], 0, L_y)
        else:
            # Target is inactive
            existence_flags[n] = 0
...
def generate_clutter_measurements():
    num_clutter = cp.random.poisson(Lambda_C)
    clutter_measurements = cp.random.uniform(
        [0, 0],
        [L_x, L_y],
        size=(int(num_clutter), 2)
    )
...
def generate_measurements(target_states, existence_flags):
    ...
    for n in range(N_max):
        if existence_flags[n]:
            # Target is active
            if cp.random.rand() <= P_D:
                # Target is detected
                position = target_states[n][:2]
                measurement_noise = cp.random.multivariate_normal(
                    mean=[0, 0],
                    cov=R
                )
...
def joint_proposal(xk_current, ek_current, xk_prev, ek_prev, particle):   #We don't use the argument particle here ??
    xk_star = cp.zeros_like(cp.array(xk_current))
    ek_star = cp.zeros_like(cp.array(ek_current))
    ...
    for n in range(N_max):
        ...
        if ekn_prev == 1:
            if ekn == 1:
                ...
                log_p_e = cp.log(1 - P_S)
        else:
            if ekn == 1:
                log_p_e = cp.log(P_B)
            else:
                log_p_e = cp.log(1 - P_B)
        # State transition probability
        if ekn == 1:
            if ekn_prev == 1:
                mean = A @ xk_prev[n]
                cov = Q
                log_p_x = multivariate_normal.logpdf(xk[n].get(), mean=mean.get(), cov=cov.get())
            else:
                # Uniform prior over surveillance area and velocities
                volume = L_x * L_y * (2 * V_max) ** 2
                log_p_x = -cp.log(volume)
        else:
...
def compute_log_likelihood(measurements_k, xk, ek):
    num_targets = cp.sum(ek)
    mu_k = Lambda_C + num_targets * P_D
    M_k = len(measurements_k)
    log_poisson = poisson.logpmf(M_k, mu_k.get())
...
def refinement_steps(xk_current, ek_current, xk_prev, ek_prev, measurements_k):
    for n in range(N_max):
        ...
        # Accept or reject
        if cp.log(cp.random.rand()) < log_alpha:
            # Accept proposal
...
def compute_log_target_density_single(
    ...
    if ekn == 1:
        if ekn_prev == 1:
            mean = A @ xk_prev[n]
            cov = Q
            log_p_x = multivariate_normal.logpdf(xkn.get(), mean=mean.get(), cov=cov.get())
        else:
            volume = L_x * L_y * (2 * V_max) ** 2
            log_p_x = -cp.log(volume)
    else:
    ...
    for k in range(K):
        for n in range(N_max):
            if target_trajectories[n][k] is not None and estimated_trajectories[n][k] is not None:
                true_pos = cp.array(target_trajectories[n][k])
                est_pos = cp.array(estimated_trajectories[n][k])
                error = cp.linalg.norm(true_pos - est_pos)
                total_error += error ** 2
                count += 1
    rmse = cp.sqrt(total_error / count) if count > 0 else None
...
def generate_clutter_measurements():
    num_clutter = int(cp.random.poisson(lam = Lambda_C))
...
def joint_proposal(xk_current, ek_current, xk_prev, ek_prev, particle):   #We don't use the argument particle here ??
    xk_star = cp.zeros_like(xk_current)
    ek_star = cp.zeros_like(ek_current)
    xk_prev_star = xk_prev.copy()
    ek_prev_star = ek_prev.copy()
...
def compute_log_likelihood(measurements_k, xk, ek):
    ...
    mu_k = Lambda_C + num_targets * P_D
    M_k = len(measurements_k)
    log_poisson = cp.array(poisson.logpmf(M_k, mu_k))
...
for k in tqdm(range(K), desc="Time Steps"):
    ...
    for n in range(N_max):
        ...
        if active_particles:
            print("Coucou")
            mean_state = cp.mean(active_particles, axis=0)
